...
export interface OAuthUvaConfig {
  [workspaceId: string]: OAuthUvaWorkspaceConfig
}

export interface OAuthUvaWorkspaceConfig {
  /**
   * OAuth Client ID
   *
...
/**
 * Event handler for generic OAuth using UvA-OIDC and PKCE.
 */
export function defineOAuthUvaEventHandler<TUser = OidcUser>(workspaceId: string, { config, onSuccess, onError }: OAuthConfig<OAuthUvaWorkspaceConfig, { user: TUser, tokens: OidcTokens }>) {
  return eventHandler(async (event: H3Event) => {
    // config = defu(config, useRuntimeConfig(event).oauth.oidc, {
    //   scope: ['openid'],
    // } satisfies OAuthOidcConfig)
    config = useRuntimeConfig(event)?.oauth?.uva[workspaceId] ?? useRuntimeConfig(event).oauth.uva['default']
    ...
    if (!config.clientId || !config.openidConfig) {
      return handleMissingConfiguration(event, 'uva', ['clientId', 'openidConfig'], onError)
    }
    ...
    if (query.state !== state) {
      return handleInvalidState(event, 'uva', onError)
    }
    ...
    if (tokens.error) {
      return handleAccessTokenErrorResponse(event, 'uva', tokens, onError)
    }
    ...
    if (tokens.id_token) {
      ...
      if (claims.nonce !== nonce) {
        const error = createError({
          statusCode: 401,
          message: 'UvA OIDC login failed: nonce mismatch',
        })
        if (!onError) throw error
        return onError(event, error)
      }
    ...
    // some UvA-OIDC providers do not support a userinfo endpoint so we only call it when its defined inside the UvA-OIDC config
